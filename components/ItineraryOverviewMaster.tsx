...
import { ItineraryOverview } from '../types';
import { initialItineraryOverviews } from './mockData';

interface ItineraryOverviewMasterProps {
  onBack: () => void;
}

const ItineraryOverviewMaster: React.FC<ItineraryOverviewMasterProps> = ({ onBack }) => {
  const [overviews, setOverviews] = useState<ItineraryOverview[]>(initialItineraryOverviews);
  const [showModal, setShowModal] = useState(false);
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  
  const [currentItem, setCurrentItem] = useState<Partial<ItineraryOverview>>({ 
    status: 'Active',
    language: 'English'
  });

  const handleEdit = (item: ItineraryOverview) => {
    setCurrentItem({ ...item });
    setShowModal(true);
  };
  ...
  const handleDelete = (id: string, e: React.MouseEvent) => {
    ...
    if (window.confirm('Delete overview?')) {
      setOverviews(prev => prev.filter(i => i.id !== id));
    }
  };
  ...
  const handleSave = () => {
    ...
    const newItem: ItineraryOverview = {
        id: currentItem.id || Math.random().toString(),
        name: currentItem.name,
        overviewInfo: currentItem.overviewInfo || '',
        highlightInfo: currentItem.highlightInfo || '',
        language: currentItem.language || 'English',
        status: currentItem.status || 'Active'
    };

    if (currentItem.id) {
        setOverviews(prev => prev.map(i => i.id === currentItem.id ? newItem : i));
    } else {
        setOverviews(prev => [...prev, newItem]);
    }
    setShowModal(false);
    resetForm();
  };
  ...
  const filteredOverviews = overviews.filter(i => {
         ...
         <div className="overflow-x-auto flex-1">
            <table className="w-full text-left">
               <thead className="bg-slate-50 border-b border-slate-100 text-slate-600">
                  <tr>
                     <th className="px-4 py-3 w-10 text-center text-xs font-bold uppercase text-blue-600">#</th>
                     <th className="px-4 py-3 w-10 text-center"><input type="checkbox" className="rounded"/></th>
                     <th className="px-4 py-3 text-xs font-bold uppercase bg-blue-600 text-white">Overview Name</th>
                     <th className="px-4 py-3 text-xs font-bold uppercase bg-blue-600 text-white">Overview Information</th>
                     <th className="px-4 py-3 text-xs font-bold uppercase bg-blue-600 text-white">Highlight Information</th>
                     <th className="px-4 py-3 text-xs font-bold uppercase bg-blue-600 text-white text-center">Language</th>
                     <th className="px-4 py-3 text-xs font-bold uppercase bg-blue-600 text-white text-right">Status</th>
                  </tr>
               </thead>
               <tbody className="divide-y divide-slate-100">
                  {filteredOverviews.map((item, index) => (
                     <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                        ...
                        <td className="px-4 py-3 text-sm text-slate-600 max-w-xs">
                            <div className="line-clamp-3" dangerouslySetInnerHTML={{ __html: item.overviewInfo }} />
                        </td>
                        <td className="px-4 py-3 text-sm text-slate-600 max-w-xs">
                            <div className="line-clamp-3" dangerouslySetInnerHTML={{ __html: item.highlightInfo }} />
                        </td>
                        <td className="px-4 py-3 text-center">
                           <button className="bg-blue-400 hover:bg-blue-500 text-white text-xs px-3 py-1 rounded-full shadow-sm transition-colors">
                              View Language
                           </button>
                        </td>
                  ...
                  {filteredOverviews.length === 0 && (
                    <tr>
                      <td colSpan={7} className="px-6 py-8 text-center text-slate-400 text-sm">No overviews found.</td>
                    </tr>
                  )}
               </tbody>
            </table>
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
               <div className="bg-slate-800 px-6 py-4 flex justify-between items-center">
                  <h3 className="text-lg font-bold text-white uppercase">Add Overview</h3>
               ...
               <div className="p-8 space-y-6 overflow-y-auto flex-1 bg-white">
                  ...
                  <div>
                      <label className="block text-xs font-semibold text-slate-500 mb-1">Overview Name</label>
                      <input 
                        type="text" 
                  ...
                  <div>
                     <label className="block text-xs font-semibold text-slate-500 mb-1">Overview Information</label>
                     <div className="border border-slate-300 rounded-lg overflow-hidden">
                        ...
                        <textarea 
                          value={currentItem.overviewInfo || ''} 
                          onChange={(e) => setCurrentItem({...currentItem, overviewInfo: e.target.value})} 
                  ...
                  <div>
                     <label className="block text-xs font-semibold text-slate-500 mb-1">Highlight Information</label>
                     <div className="border border-slate-300 rounded-lg overflow-hidden">
                        <div className="bg-white px-3 py-2 border-b border-slate-300 flex gap-4 text-slate-600 text-xs">
                           <span>File</span><span>Edit</span><span>View</span><span>Insert</span><span>Format</span>
                        </div>
                        ...
                        <textarea 
                          value={currentItem.highlightInfo || ''} 
                          onChange={(e) => setCurrentItem({...currentItem, highlightInfo: e.target.value})} 
                          className="w-full px-3 py-2 border-0 text-sm focus:ring-0 h-40 resize-none"
                        />
                         <div className="bg-slate-50 border-t border-slate-300 px-3 py-1 text-[10px] text-slate-400 text-right">Powered by TinyMCE</div>
                     </div>
...
export default ItineraryOverviewMaster;
