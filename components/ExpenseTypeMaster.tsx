...
import { initialExpenseTypes, initialExpenseHeads } from './mockData';

interface ExpenseTypeMasterProps {
  onBack: () => void;
}

const ExpenseTypeMaster: React.FC<ExpenseTypeMasterProps> = ({ onBack }) => {
  const [expenseTypes, setExpenseTypes] = useState<ExpenseType[]>(initialExpenseTypes);
  const [showModal, setShowModal] = useState(false);
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  const [currentType, setCurrentType] = useState<Partial<ExpenseType>>({ status: 'Active' });
  ...
  const handleSave = () => {
    if (!currentType.name || !currentType.expenseHeadId) return;

    const headName = initialExpenseHeads.find(h => h.id === currentType.expenseHeadId)?.name || '';

    const newItem: ExpenseType = {
      id: currentType.id || Math.random().toString(),
      expenseHeadId: currentType.expenseHeadId,
    ...
    if (currentType.id) {
      setExpenseTypes(prev => prev.map(t => t.id === currentType.id ? newItem : t));
    } else {
      setExpenseTypes(prev => [...prev, newItem]);
    }
    setShowModal(false);
    ...
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="flex justify-between items-center mb-6 p-4 bg-white border-b border-slate-200">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl font-bold text-slate-800">Expense Type</h2>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredTypes.map((item, index) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">{item.expenseHeadName}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden">
            <div className="bg-slate-800 px-6 py-4 flex justify-between items-center text-white">
              <h3 className="font-bold">{currentType.id ? 'Edit Expense Type' : 'Add Expense Type'}</h3>
              <button onClick={() => setShowModal(false)}><X size={20}/></button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                 <label className="block text-xs font-semibold text-slate-500 mb-1">Expense Head</label>
                 <select value={currentType.expenseHeadId} onChange={e => setCurrentType({...currentType, expenseHeadId: e.target.value})} className="w-full border border-slate-300 rounded px-3 py-2 text-sm border-l-4 border-l-red-500">
                    <option value="">Select</option>
                    {initialExpenseHeads.map(h => <option key={h.id} value={h.id}>{h.name}</option>)}
                 </select>
              </div>
              <div>
                 <label className="block text-xs font-semibold text-slate-500 mb-1">Expense Type</label>
