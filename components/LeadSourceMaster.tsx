...
import { initialLeadSources } from './mockData';

interface LeadSourceMasterProps {
  onBack: () => void;
}

const LeadSourceMaster: React.FC<LeadSourceMasterProps> = ({ onBack }) => {
  ...
  const filteredLeadSources = leadSources.filter(ls => {
    return (leadSourceStatusFilter === 'All' || ls.status === leadSourceStatusFilter) &&
           ls.name.toLowerCase().includes(leadSourceSearch.toLowerCase());
  });

  return (
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4"><div className="flex items-center gap-4"><button onClick={onBack} className="p-2 hover:bg-slate-200 rounded-full transition-colors text-slate-500"><ArrowLeft size={20} /></button><div><h2 className="text-2xl font-bold text-slate-800">Lead Source Master</h2><p className="text-sm text-slate-500">Manage lead sources for better tracking</p></div></div><div className="flex items-center gap-3"><button className="bg-white border border-slate-300 text-slate-600 px-3 py-2 rounded-lg flex items-center gap-2 hover:bg-slate-50 text-sm font-medium shadow-sm"><Download size={16}/> Sample</button><label className="bg-white border border-slate-300 text-slate-600 px-3 py-2 rounded-lg flex items-center gap-2 hover:bg-slate-50 text-sm font-medium shadow-sm cursor-pointer"><Upload size={16}/> Import<input type="file" className="hidden" accept=".csv,.xlsx"/></label><button onClick={() => { setNewLeadSource({ status: 'Active' }); setShowAddLeadSourceModal(true); }} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 shadow-sm text-sm font-medium"><Plus size={18} /> Add Lead Source</button></div></div>
