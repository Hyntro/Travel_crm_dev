...
import { FitInclusionMaster } from '../types';
import { initialFitInclusions, initialDestinations } from './mockData';

interface FitInclusionMasterProps {
  onBack: () => void;
}

const FitInclusionMaster: React.FC<FitInclusionMasterProps> = ({ onBack }) => {
  const [inclusions, setInclusions] = useState<FitInclusionMaster[]>(initialFitInclusions);
  const [showModal, setShowModal] = useState(false);
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  
  const [currentItem, setCurrentItem] = useState<Partial<FitInclusionMaster>>({ 
    status: 'Active',
    language: 'English',
    isDefault: false
  });

  const handleEdit = (item: FitInclusionMaster) => {
    setCurrentItem({ ...item });
    setShowModal(true);
  };
  ...
  const handleSave = () => {
    ...
    const newItem: FitInclusionMaster = {
    ...
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800">FIT Inculsions I Exculsions I T & C I Cancellation Policy</h2>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 mb-6 flex flex-wrap gap-4 items-end justify-between">
         ...
         <div>
             ...
             >
               <Plus size={18} /> Add FIT Inculsions I Exculsions I T & C I Cancellation Policy
             </button>
         </div>
      </div>
            ...
            <table className="w-full text-left">
               <thead className="bg-slate-50 border-b border-slate-100 text-slate-600">
                  <tr>
                     ...
                     <th className="px-4 py-3 text-xs font-bold uppercase w-1/3">Inclusion</th>
                     <th className="px-4 py-3 text-xs font-bold uppercase w-1/3">Exclusion</th>
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
               <div className="bg-slate-800 px-6 py-4 flex justify-between items-center">
                  <h3 className="text-lg font-bold text-white uppercase">Add FIT Inculsions I Exculsions</h3>
               ...
               <div className="p-8 space-y-6 overflow-y-auto flex-1 bg-white">
                  ...
                  <div>
                      ...
                      <input 
                        ...
                        className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 border-b-2 border-b-red-500"
                        placeholder="Enter Destination or Select All"
                      />
                  </div>
                  ...
                  <div>
                     ...
                     <div className="border border-slate-300 rounded-lg overflow-hidden">
                         {/* Using same rich text toolbar logic */}
                        <div className="bg-white px-3 py-2 border-b border-slate-300 flex gap-4 text-slate-600 text-xs">
                           <span>File</span><span>Edit</span><span>View</span><span>Insert</span><span>Format</span><span>Tools</span><span>Table</span><span>Help</span>
                        </div>
                        <div className="px-3 py-2 flex gap-3 text-slate-500 border-b border-slate-100 bg-slate-50">
...
export default FitInclusionMaster;
