...
import { RestaurantMealPlan } from '../types';
import { initialRestaurantMealPlans } from './mockData';

interface RestaurantMealPlanMasterProps {
  onBack: () => void;
}

const RestaurantMealPlanMaster: React.FC<RestaurantMealPlanMasterProps> = ({ onBack }) => {
  const [mealPlans, setMealPlans] = useState<RestaurantMealPlan[]>(initialRestaurantMealPlans);
  const [showModal, setShowModal] = useState(false);
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  const [currentMealPlan, setCurrentMealPlan] = useState<Partial<RestaurantMealPlan>>({ status: 'Active' });

  const handleEdit = (plan: RestaurantMealPlan) => {
  ...
  const handleSave = () => {
    ...
    if (currentMealPlan.id) {
      setMealPlans(prev => prev.map(m => m.id === currentMealPlan.id ? { ...m, ...currentMealPlan } as RestaurantMealPlan : m));
    } else {
      const newPlan: RestaurantMealPlan = {
    ...
    setShowModal(false);
    setCurrentMealPlan({ status: 'Active' });
  };
    ...
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800">Restaurants Meal Plan</h2>
            <p className="text-sm text-slate-500">Manage meal types for restaurants</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
           <button 
             onClick={() => { setCurrentMealPlan({ status: 'Active' }); setShowModal(true); }} 
           ...
           >
             <Plus size={18} /> Add Restaurant Meal Plan
           </button>
        </div>
      </div>
            ...
            <table className="w-full text-left">
               ...
               <tbody className="divide-y divide-slate-100">
                  ...
                  {filteredMealPlans.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-6 py-8 text-center text-slate-400 text-sm">No meal plans found.</td>
                    </tr>
                  )}
               </tbody>
            </table>
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
               <div className="bg-slate-800 px-6 py-4 flex justify-between items-center">
                  <h3 className="text-lg font-bold text-white">{currentMealPlan.id ? 'Edit Restaurant Meal Name' : 'Add Restaurant Meal Name'}</h3>
...
export default RestaurantMealPlanMaster;
