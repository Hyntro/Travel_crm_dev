...
import { initialBusinessTypes } from './mockData';

interface BusinessTypeMasterProps {
  onBack: () => void;
}

const BusinessTypeMaster: React.FC<BusinessTypeMasterProps> = ({ onBack }) => {
  ...
  const filteredBusinessTypes = businessTypes.filter(bt => {
    return (businessTypeStatusFilter === 'All' || bt.status === businessTypeStatusFilter) &&
           bt.name.toLowerCase().includes(businessTypeSearch.toLowerCase());
  });
