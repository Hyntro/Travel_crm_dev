...
import { initialTransferTypes } from './mockData';

interface TransferTypeMasterProps {
  onBack: () => void;
}

const TransferTypeMaster: React.FC<TransferTypeMasterProps> = ({ onBack }) => {
  const [transferTypes, setTransferTypes] = useState<TransferType[]>(initialTransferTypes);
  const [showModal, setShowModal] = useState(false);
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  const [currentType, setCurrentType] = useState<Partial<TransferType>>({ status: 'Active' });
  ...
  const handleDelete = (id: string, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (window.confirm('Delete transfer type?')) {
      setTransferTypes(prev => prev.filter(t => t.id !== id));
    }
  };
  ...
  const handleSave = () => {
    ...
    if (currentType.id) {
      setTransferTypes(prev => prev.map(t => t.id === currentType.id ? { ...t, ...currentType } as TransferType : t));
    } else {
      ...
      setTransferTypes(prev => [...prev, newType]);
    }
    setShowModal(false);
    setCurrentType({ status: 'Active' });
  };

  const filteredTypes = transferTypes.filter(t => {
    ...
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800">Transfer Type Master</h2>
            <p className="text-sm text-slate-500">Manage categories of transfers</p>
          </div>
        </div>
        <button 
          onClick={() => { setCurrentType({ status: 'Active' }); setShowModal(true); }} 
        ...
        >
          <Plus size={18} /> Add Transfer Type
        </button>
      </div>
            ...
            <table className="w-full text-left">
               <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                     ...
                     <th className="px-6 py-3 text-xs font-bold text-slate-500 uppercase">Transfer Type Name</th>
               ...
               <tbody className="divide-y divide-slate-100">
                  ...
                  {filteredTypes.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-6 py-8 text-center text-slate-400 text-sm">No transfer types found.</td>
                    </tr>
                  )}
               </tbody>
            </table>
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
               <div className="bg-slate-800 px-6 py-4 flex justify-between items-center">
                  <h3 className="text-lg font-bold text-white">{currentType.id ? 'Edit Transfer Type' : 'Add Transfer Type'}</h3>
...
export default TransferTypeMaster;
