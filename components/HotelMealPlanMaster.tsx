...
import { HotelMealPlan } from '../types';
import { initialHotelMealPlans } from './mockData';

interface HotelMealPlanMasterProps {
  onBack: () => void;
}

const HotelMealPlanMaster: React.FC<HotelMealPlanMasterProps> = ({ onBack }) => {
  const [mealPlans, setMealPlans] = useState<HotelMealPlan[]>(initialHotelMealPlans);
  const [showModal, setShowModal] = useState(false);
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  const [currentMealPlan, setCurrentMealPlan] = useState<Partial<HotelMealPlan>>({ status: 'Active', isDefault: false });

  const handleEdit = (plan: HotelMealPlan) => {
  ...
  const handleSave = () => {
    if (!currentMealPlan.name) return;

    if (currentMealPlan.id) {
      setMealPlans(prev => prev.map(m => m.id === currentMealPlan.id ? { ...m, ...currentMealPlan } as HotelMealPlan : m));
    } else {
      const newPlan: HotelMealPlan = {
        id: Math.random().toString(),
        name: currentMealPlan.name,
        voucherName: currentMealPlan.voucherName || '',
        isDefault: currentMealPlan.isDefault || false,
        status: currentMealPlan.status || 'Active',
        createdBy: 'Demo',
        modifiedBy: 'Demo'
      };
    ...
    setShowModal(false);
    setCurrentMealPlan({ status: 'Active', isDefault: false });
  };
    ...
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800">Hotel Meal Plan</h2>
            <p className="text-sm text-slate-500">Manage meal plans and voucher display names</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
           ...
           <button 
             onClick={() => { setCurrentMealPlan({ status: 'Active', isDefault: false }); setShowModal(true); }} 
            ...
            <table className="w-full text-left">
               <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                     ...
                     <th className="px-6 py-3 text-xs font-bold text-slate-500 uppercase">Meal Name</th>
                     <th className="px-6 py-3 text-xs font-bold text-slate-500 uppercase">Display Voucher</th>
               ...
               <tbody className="divide-y divide-slate-100">
                  {filteredMealPlans.map((plan, index) => (
                     <tr key={plan.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-sm text-slate-600">{plan.voucherName}</td>
                        <td className="px-6 py-4 text-xs text-slate-500">{plan.createdBy}</td>
                        <td className="px-6 py-4 text-xs text-slate-500">{plan.modifiedBy}</td>
                        ...
                        <td className="px-6 py-4 text-right">
                           <div className="flex items-center justify-end gap-2">
                              <button onClick={() => handleEdit(plan)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded">
                                <Edit2 size={16}/>
                              </button>
                              <button onClick={(e) => handleDelete(plan.id, e)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded">
                                <Trash2 size={16}/>
                              </button>
                           </div>
                        </td>
                  ...
                  {filteredMealPlans.length === 0 && (
                    <tr>
                      <td colSpan={7} className="px-6 py-8 text-center text-slate-400 text-sm">No meal plans found.</td>
                    </tr>
                  )}
               </tbody>
            </table>
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
               <div className="bg-slate-800 px-6 py-4 flex justify-between items-center">
                  <h3 className="text-lg font-bold text-white">{currentMealPlan.id ? 'Edit Meal Plan' : 'Add Meal Plan'}</h3>
               ...
               <div className="p-6 space-y-4">
                  <div>
                      <label className="block text-xs font-semibold text-slate-500 mb-1">Meal Name</label>
                      <input 
                        type="text" 
                  ...
                  <div>
                      <label className="block text-xs font-semibold text-slate-500 mb-1">Display Name in Voucher</label>
                      <input 
                        type="text" 
                        value={currentMealPlan.voucherName || ''} 
                        onChange={(e) => setCurrentMealPlan({...currentMealPlan, voucherName: e.target.value})} 
                        className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                      <div className="h-0.5 w-8 bg-red-500 mt-1"></div>
                  </div>
                  <div>
                     ...
                     <select 
                        value={currentMealPlan.status} 
                        onChange={(e) => setCurrentMealPlan({...currentMealPlan, status: e.target.value as any})} 
                  ...
                  <div className="flex items-center gap-2 pt-2">
                     <input 
                       type="checkbox" 
                       id="setDefault"
                       checked={currentMealPlan.isDefault || false}
                       onChange={(e) => setCurrentMealPlan({...currentMealPlan, isDefault: e.target.checked})}
...
export default HotelMealPlanMaster;
