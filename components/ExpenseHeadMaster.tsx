...
import { initialExpenseHeads } from './mockData';

interface ExpenseHeadMasterProps {
  onBack: () => void;
}

const ExpenseHeadMaster: React.FC<ExpenseHeadMasterProps> = ({ onBack }) => {
  const [expenseHeads, setExpenseHeads] = useState<ExpenseHead[]>(initialExpenseHeads);
  const [showModal, setShowModal] = useState(false);
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  const [currentHead, setCurrentHead] = useState<Partial<ExpenseHead>>({ status: 'Active' });

  const handleEdit = (item: ExpenseHead) => {
    setCurrentHead({ ...item });
    setShowModal(true);
  };

  const handleDelete = (id: string) => {
    if (window.confirm('Delete expense head?')) {
  ...
  const handleSave = () => {
    ...
    const newItem: ExpenseHead = {
      id: currentHead.id || Math.random().toString(),
    ...
    if (currentHead.id) {
      setExpenseHeads(prev => prev.map(h => h.id === currentHead.id ? newItem : h));
    } else {
      setExpenseHeads(prev => [...prev, newItem]);
    }
    setShowModal(false);
    setCurrentHead({ status: 'Active' });
  };

  const filteredHeads = expenseHeads.filter(h => {
    ...
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="flex justify-between items-center mb-6 p-4 bg-white border-b border-slate-200">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl font-bold text-slate-800">Expense Head</h2>
        </div>
        <button onClick={() => { setCurrentHead({ status: 'Active' }); setShowModal(true); }} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
          <Plus size={18} /> Add Expense Head
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden">
            <div className="bg-slate-800 px-6 py-4 flex justify-between items-center text-white">
              <h3 className="font-bold">{currentHead.id ? 'Edit Expense Head' : 'Add Expense Head'}</h3>
              <button onClick={() => setShowModal(false)}><X size={20}/></button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                 <label className="block text-xs font-semibold text-slate-500 mb-1">Expense Head</label>
                 <input type="text" value={currentHead.name || ''} onChange={e => setCurrentHead({...currentHead, name: e.target.value})} className="w-full border border-slate-300 rounded px-3 py-2 text-sm border-l-4 border-l-red-500"/>
              </div>
              <div>
                 <label className="block text-xs font-semibold text-slate-500 mb-1">Status</label>
                 <select value={currentHead.status} onChange={e => setCurrentHead({...currentHead, status: e.target.value as any})} className="w-full border border-slate-300 rounded px-3 py-2 text-sm">
                    <option value="Active">Active</option>
                    <option value="Inactive">Inactive</option>
                 </select>
              </div>
