...
import { initialFlightSeatClasses } from './mockData';

interface FlightSeatClassMasterProps {
  onBack: () => void;
}

const FlightSeatClassMaster: React.FC<FlightSeatClassMasterProps> = ({ onBack }) => {
  const [seatClasses, setSeatClasses] = useState<FlightSeatClass[]>(initialFlightSeatClasses);
  const [showModal, setShowModal] = useState(false);
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  const [currentClass, setCurrentClass] = useState<Partial<FlightSeatClass>>({ status: 'Active' });

  const handleEdit = (cls: FlightSeatClass) => {
    setCurrentClass({ ...cls });
    setShowModal(true);
  };
  ...
  const handleDelete = (id: string, e: React.MouseEvent) => {
    ...
    if (window.confirm('Delete seat class?')) {
      setSeatClasses(prev => prev.filter(c => c.id !== id));
    }
  };
  ...
  const handleSave = () => {
    ...
    const newClass: FlightSeatClass = {
        id: currentClass.id || Math.random().toString(),
    ...
    if (currentClass.id) {
        setSeatClasses(prev => prev.map(c => c.id === currentClass.id ? newClass : c));
    } else {
        setSeatClasses(prev => [...prev, newClass]);
    }
    setShowModal(false);
    setCurrentClass({ status: 'Active' });
  };

  const filteredClasses = seatClasses.filter(c => {
    ...
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800">Flight Seat Class Master</h2>
            <p className="text-sm text-slate-500">Manage flight seat categories</p>
          </div>
        </div>
        <button 
          onClick={() => { setCurrentClass({ status: 'Active' }); setShowModal(true); }} 
        ...
        >
          <Plus size={18} /> Add Flight Seat Class
        </button>
      </div>
            ...
            <table className="w-full text-left">
               <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                     ...
                     <th className="px-6 py-3 text-xs font-bold text-slate-500 uppercase">Flight Seat Class</th>
               ...
               <tbody className="divide-y divide-slate-100">
                  {filteredClasses.map((cls, index) => (
                     <tr key={cls.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-xs text-slate-500">{cls.createdBy}</td>
                        <td className="px-6 py-4 text-xs text-slate-500">{cls.modifiedBy}</td>
                        <td className="px-6 py-4">
                           <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${cls.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                              {cls.status}
                           </span>
                        </td>
                        <td className="px-6 py-4 text-right">
                           <div className="flex items-center justify-end gap-2">
                              <button onClick={() => handleEdit(cls)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded">
                                <Edit2 size={16}/>
                              </button>
                              <button onClick={(e) => handleDelete(cls.id, e)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded">
                                <Trash2 size={16}/>
                              </button>
                           </div>
                        </td>
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
               <div className="bg-slate-800 px-6 py-4 flex justify-between items-center">
                  <h3 className="text-lg font-bold text-white">{currentClass.id ? 'Edit Flight Seat Class' : 'Add Flight Seat Class'}</h3>
               ...
               <div className="p-6 space-y-4">
                  ...
                  <div>
                     ...
                     <select 
                        ...
                        onChange={(e) => setCurrentClass({...currentClass, status: e.target.value as any})} 
...
export default FlightSeatClassMaster;
