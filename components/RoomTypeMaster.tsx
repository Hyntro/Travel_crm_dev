...
import { initialRoomTypes } from './mockData';

interface RoomTypeMasterProps {
  onBack: () => void;
}

const RoomTypeMaster: React.FC<RoomTypeMasterProps> = ({ onBack }) => {
  const [roomTypes, setRoomTypes] = useState<RoomType[]>(initialRoomTypes);
  const [showModal, setShowModal] = useState(false);
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  const [currentRoom, setCurrentRoom] = useState<Partial<RoomType>>({ status: 'Active' });
  ...
  const handleSave = () => {
    ...
    if (currentRoom.id) {
      setRoomTypes(prev => prev.map(r => r.id === currentRoom.id ? { ...r, ...currentRoom } as RoomType : r));
    } else {
    ...
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800">Room Type Master</h2>
            <p className="text-sm text-slate-500">Manage room configurations</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
           ...
           <button 
             onClick={() => { setCurrentRoom({ status: 'Active' }); setShowModal(true); }} 
            ...
            <table className="w-full text-left">
               <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                     ...
                     <th className="px-6 py-3 text-xs font-bold text-slate-500 uppercase">Room Type Name</th>
               ...
               <tbody className="divide-y divide-slate-100">
                  {filteredRoomTypes.map((room, index) => (
                     <tr key={room.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-xs text-slate-500">{room.createdBy}</td>
                        <td className="px-6 py-4 text-xs text-slate-500">{room.modifiedBy}</td>
                        <td className="px-6 py-4">
                           <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${room.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                              {room.status}
                           </span>
                        </td>
                        <td className="px-6 py-4 text-right">
                           <div className="flex items-center justify-end gap-2">
                              <button onClick={() => handleEdit(room)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded">
                                <Edit2 size={16}/>
                              </button>
                              <button onClick={(e) => handleDelete(room.id, e)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded">
                                <Trash2 size={16}/>
                              </button>
                           </div>
                        </td>
                  ...
                  {filteredRoomTypes.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-6 py-8 text-center text-slate-400 text-sm">No room types found.</td>
                    </tr>
                  )}
               </tbody>
            </table>
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
               <div className="bg-slate-800 px-6 py-4 flex justify-between items-center">
                  <h3 className="text-lg font-bold text-white">{currentRoom.id ? 'Edit Room Type' : 'Add Room Type'}</h3>
               ...
               <div className="p-6 space-y-4">
                  ...
                  <div>
                      <label className="block text-xs font-semibold text-slate-500 mb-1">Room Information</label>
                      <input 
                        type="text" 
                        value={currentRoom.info || ''} 
                        onChange={(e) => setCurrentRoom({...currentRoom, info: e.target.value})} 
                        className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                  </div>
                  <div>
