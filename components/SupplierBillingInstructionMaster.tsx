...
import { SupplierBillingInstruction } from '../types';
import { initialSupplierBillingInstructions } from './mockData';

interface SupplierBillingInstructionMasterProps {
  onBack: () => void;
}

const SupplierBillingInstructionMaster: React.FC<SupplierBillingInstructionMasterProps> = ({ onBack }) => {
  const [instructions, setInstructions] = useState<SupplierBillingInstruction[]>(initialSupplierBillingInstructions);
  const [showModal, setShowModal] = useState(false);
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  
  const [currentItem, setCurrentItem] = useState<Partial<SupplierBillingInstruction>>({ 
    status: 'Active',
    isDefault: false
  });

  const handleEdit = (item: SupplierBillingInstruction) => {
    setCurrentItem({ ...item });
    setShowModal(true);
  };
  ...
  const handleSave = () => {
    ...
    const newItem: SupplierBillingInstruction = {
    ...
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800">Supplier Billing Instruction</h2>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 mb-6 flex flex-wrap gap-4 items-end justify-between">
         ...
         <div>
             ...
             >
               <Plus size={18} /> Add Supplier Billing Instrunction
             </button>
         </div>
      </div>
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
               <div className="bg-slate-800 px-6 py-4 flex justify-between items-center">
                  <h3 className="text-lg font-bold text-white uppercase">{currentItem.id ? 'Edit Supplier Billing Instrunction' : 'Add Supplier Billing Instrunction'}</h3>
               ...
               <div className="p-8 space-y-6 overflow-y-auto flex-1 bg-white">
                  <div>
                     <label className="block text-xs font-semibold text-slate-500 mb-1">Supplier Billing Instruction</label>
...
export default SupplierBillingInstructionMaster;
